...
On iOS 27 LaunchServices no longer registers from an Info.plist dictionary: the call only logs that it is deprecated. When that leaves no record of the bundle's build, the bundle is registered through LaunchServices' containerized interface, as a system app that cannot be deleted from the Home Screen, and the record is read back. A record of the current build that lacks only the settings-bundle mark is kept. lsd accepts that interface only from callers its embedded-registration check lets through, which the vphone firmware patches to allow; elsewhere the registration fails. `uicache` from uikittools 2.1.8 and earlier does not use this interface, so it cannot register apps on iOS 27. Unregistering is unchanged and passes the same lsd check.
