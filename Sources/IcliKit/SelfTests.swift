...
public func runSelfTests(expectedLayout: String? = nil, registrationFixture: String? = nil) throws -> [String: Any] {
    ...
    return [
        ...
        "not_tested": [
            ...
            "clipboard and device setting changes",
...
private func selfTestRegisteredApps() throws -> [[String: Any]] {
    guard let raw = takeCString(icli_apps_json()),
          let apps = try JSONSerialization.jsonObject(with: Data(raw.utf8)) as? [[String: Any]]
    else {
...
private func selfTestKeychain() throws -> [String: Any] {
    ...
    let remaining = try listKeychain(
        className: "generic",
        service: service,
        account: account,
        server: nil,
