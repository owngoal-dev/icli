...
char *icli_bootlogo_render_json(
    ...
    if (width <= 0 || height <= 0 || width > 16384 || height > 16384)
        return icli_json(@{@"error": @"screen pixel size unavailable; pass --width and --height"});
    ...
    if (fittedWidth < 1 || fittedHeight < 1) {
        CGImageRelease(mark);
        return icli_json(@{@"error": @"mark image is too small to render"});
    }
    ...
    CGContextDrawImage(
        context,
        CGRectMake(floor((width - fittedWidth) / 2), floor((height - fittedHeight) / 2), fittedWidth, fittedHeight),
        mark
    );
    ...
    if (destination) {
        CGImageDestinationAddImage(
            destination,
            image,
            (__bridge CFDictionaryRef)@{(__bridge NSString *)kCGImageDestinationLossyCompressionQuality: @1.0}
        );
    ...
    NSError *error = nil;
    if (![data writeToFile:@(output_path) options:NSDataWritingAtomic error:&error])
        return icli_json(@{@"error": error.localizedDescription ?: @"boot logo could not be written"});
    return icli_json(@{
        @"path": @(output_path),
        @"width": @(width),
        @"height": @(height),
        ...
        @"mark_pixels": @[@(fittedWidth), @(fittedHeight)],
        @"bytes": @(data.length),
        @"format": @"public.jpeg-2000"
    });
}
